...
if __name__ == "__main__":
    while True:
        ...
        if exit_code == 0:
            ...
            print(
                "--- Guardian: main.py exited cleanly. Restarting after a short delay... ---"
            )
        else:
            print(
                f"--- Guardian: main.py crashed with exit code {exit_code}. Performing recovery! ---"
            )
            ...
            try:
                ...
                with open("SOS.log", "a", encoding="utf-8") as f:
                    f.write(
                        f"{timestamp} - CRITICAL: main.py crashed with exit code {exit_code}. Performing git reset.\n"
                    )
            except Exception as e:
            ...
            try:
                print(
                    "Guardian: Attempting to roll back changes with 'git reset --hard HEAD'..."
                )
                ...
                print(
                    f"Guardian: CRITICAL FAILURE - Git reset failed with exit code {e.returncode}."
                )
                print(f"Guardian: Stderr: {e.stderr}")
                print(
                    "Guardian: Cannot recover automatically. Please intervene manually. Exiting."
                )
                sys.exit(1)  # Exit guardian if it can't recover
            except FileNotFoundError:
                print(
                    "Guardian: CRITICAL FAILURE - 'git' command not found. Cannot perform rollback."
                )
                print(
                    "Guardian: Cannot recover automatically. Please intervene manually. Exiting."
                )
