...
class LongTermMemory:
    ...
    def __init__(
        ...
        collection_name: str = "sophia_memories",
    ):
        try:
    ...
    def add_memory(self, memory_text: str, metadata: Optional[dict] = None):
        ...
        try:
            ...
            if not (
                ...
                raise ValueError(
                    f"Embedding must be a list of floats/ints, got: {type(embedding)} with sample {embedding[:5]}"
                )
            ...
            print(
                f"INFO: Memory '{memory_text[:30]}...' added to LTM with ID {memory_id}."
            )
        except Exception as e:
    ...
    def fetch_relevant_memories(self, query_text: str, num_results: int = 3) -> list:
        ...
        try:
            ...
            if not (
                isinstance(query_embedding, list)
                and all(isinstance(x, (float, int)) for x in query_embedding)
            ):
                raise ValueError(
                    f"Query embedding must be a list of floats/ints, got: {type(query_embedding)} with sample {query_embedding[:5]}"
                )
