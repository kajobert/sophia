...
class MockLLM(Runnable):
    ...
    def invoke(self, *args, **kwargs):
        ...
        if "Odpověz POUZE ve formátu JSON" in prompt:
            print("--- MOCK LLM: Returning a pre-defined plan with Final Answer prefix. ---")
            mock_plan_dict = {
                "plan": [
                    {"step": 1, "description": "Analyzuj kód v `main.py` aby si pochopil jeho funkcionalitu."},
                    {"step": 2, "description": "Na základe analýzy navrhni obsah pre README.md súbor."},
                    {"step": 3, "description": "Ulož navrhnutý obsah do súboru `navrh_readme.md`."}
                ]
            }
            ...
            print("--- MOCK LLM: Returning a generic Final Answer for execution step. ---")
...
if os.getenv("USE_MOCK_LLM") == "true":
    ...
    if not gemini_api_key:
        raise ValueError("GEMINI_API_KEY environment variable not set and not using mock LLM. Please set it in your .env file.")
