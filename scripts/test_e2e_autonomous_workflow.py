    ...
    try:
        ...
        try:
            ...
            result = github.list_issues(
                context, owner="ShotyCZ", repo="sophia", state="open", per_page=5
            )
...
def test_jules_plugin():
    ...
    try:
        ...
        try:
            ...
            print("✅ Jules API accessible")
            print(f"   Available sources: {len(sources.sources)}")

            # Look for sophia source
            sophia_source = next((s for s in sources.sources if "sophia" in s.name.lower()), None)
...
def test_performance_monitor():
    ...
    try:
        ...
        perf.log_tool_usage(
            context, tool_name="test_e2e_workflow", method_name="validate_plugins", success=True
        )
