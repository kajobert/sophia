...
async def main():
    ...
    for i, scenario in enumerate(TEST_SCENARIOS, 1):
        ...
        for run_num in range(1, RUNS_PER_SCENARIO + 1):
            ...
            if result["error"]:
                ...
                print(
                    f"    {status} {result['response_time']:.2f}s | "
    ...
    print(
        f"  Successful: {stats['successful_runs']} ({stats['successful_runs']/stats['total_runs']*100:.1f}%)"
    )
    print(
        f"  Failed: {stats['failed_runs']} ({stats['failed_runs']/stats['total_runs']*100:.1f}%)"
    )
    ...
    for scenario_name, scenario_stats in stats["scenario_stats"].items():
        ...
        print(
            f"    Avg response time: {scenario_stats['avg_response_time']:.2f}s ± {scenario_stats['response_time_std']:.2f}s"
        )
    ...
    for scenario_name, scenario_stats in stats["scenario_stats"].items():
        if scenario_stats["success_rate"] < 1.0:
            ...
            print("    Recommendation: Review response quality and keyword detection")

        if scenario_stats["consistency_score"] < 0.7:
            issues_found = True
            print(f"\n  ⚠️  {scenario_name}:")
            print(f"    Problem: Low consistency ({scenario_stats['consistency_score']:.2f})")
            print("    Recommendation: Investigate response variability")
