...
result = subprocess.run(
    ["python", "run.py", "Sophie, use Jules API to list all my coding sessions. Show me the results."],
...
result2 = subprocess.run(
    ["python", "run.py", "What can you tell me about Jules API? What methods are available in the tool_jules plugin?"],
    cwd=str(project_root),
    capture_output=True,
    text=True,
    timeout=60
...
result3 = subprocess.run(
    ["python", "run.py", "Sophie, explain step-by-step how to use Jules API to create a new coding session. What parameters are needed?"],
    cwd=str(project_root),
    capture_output=True,
    text=True,
    timeout=60
