...
"""
    ...
    try:
        ...
        for i in range(max_checks):
            ...
            print(
                f"   Check {i+1}/{max_checks}: {status.state} (Completed: {status.is_completed}, Error: {status.is_error})"
            )
...
async def test_sophia_discovers_plugin(
    context: SharedContext, plugin_spec: Dict[str, str], session_id: str
):
    ...
    if plugin_path.exists():
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(plugin_spec["plugin_name"], plugin_path)
            ...
            print("⚠️  Plugin module loaded but no plugin class found")
            return None
        ...
        print("⚠️  Plugin file not found yet")
...
async def test_sophia_uses_plugin(
    context: SharedContext, plugin_class, plugin_spec: Dict[str, str], all_plugins: Dict
):
    ...
    try:
        ...
        if "weather" in plugin_spec["plugin_name"]:
            if hasattr(plugin, "get_weather") or hasattr(plugin, "execute"):
                ...
                print("   (Plugin ready to use - actual API call skipped in test)")
...
async def main():
    ...
    session_id = await test_jules_creates_plugin(context, jules_api, jules_monitor, plugin_spec)
    ...
    if not plugin_class:
        ...
        print("   3. Re-run this test to verify plugin works")
        return
