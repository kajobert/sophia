...
async def launch_worker(worker_config: dict) -> dict:
    ...
    if not api_key:
        print(f"   ❌ JULES_API_KEY not set in environment")
    ...
    try:
        ...
        if response.status_code == 200 or response.status_code == 201:
            ...
            print(f"   {error_msg}")
            return {"success": False, "worker": worker_config['name'], "error": f"HTTP {response.status_code}"}
...
def update_active_sessions(workers: list):
    ...
    content = f"# Jules Active Sessions\n\n"
    ...
    for worker in workers:
        ...
        content += f"- **Check:** Morning 9:00 AM\n\n"
