...
def create_status_bar(cpu_percent: float = 0, mem_percent: float = 0, 
                      network_active: bool = True, jules_state: str = "IDLE") -> Panel:
...
def create_metrics_panel(tokens: int = 0, cost: float = 0.0, messages: int = 0,
                        cpu: float = 0, mem: float = 0) -> Panel:
...
def create_jules_panel(state: str = "IDLE", quota_used: int = 15, quota_total: int = 100,
                      session_id: str = "", branch: str = "") -> Panel:
...
async def run_demo():
    ...
    with Live(layout, console=console, refresh_per_second=10, screen=False) as live:
        ...
        layout["jules"].update(create_jules_panel(jules_state, 15, 100, jules_session, jules_branch))
        ...
        activity_log.append((datetime.now().strftime("%H:%M:%S"), "INFO", "Generating response..."))
        layout["logs"].update(create_activity_log(activity_log))
        live.refresh()
        ...
        messages += 1
        activity_log.append((datetime.now().strftime("%H:%M:%S"), "INFO", "User query about Jules"))
        ...
        activity_log.append((datetime.now().strftime("%H:%M:%S"), "INFO", "Checking Jules status..."))
        layout["logs"].update(create_activity_log(activity_log))
        live.refresh()
        ...
        show_user_message(conversation, "This looks amazing! Is this really the future of AI interfaces?")
        messages += 1
        ...
        activity_log.append((datetime.now().strftime("%H:%M:%S"), "INFO", "Generating final response..."))
        activity_log.append((datetime.now().strftime("%H:%M:%S"), "WARNING", "Jules task COMPLETED!"))
        layout["logs"].update(create_activity_log(activity_log))
        live.refresh()
        ...
        tokens += 200
        cost += 0.0008
        activity_log.append((datetime.now().strftime("%H:%M:%S"), "INFO", "Conversation completed"))
        activity_log.append((datetime.now().strftime("%H:%M:%S"), "INFO", "All panels updated successfully"))
