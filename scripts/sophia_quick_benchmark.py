...
async def main():
    ...
    for i, scenario in enumerate(TEST_SCENARIOS, 1):
        ...
        for run_num in range(1, RUNS_PER_SCENARIO + 1):
            ...
            if result["error"]:
                ...
                print(
                    f"  Run {run_num}: {status} {result['response_time']:.2f}s | "
    ...
    for scenario_name, results in all_results.items():
        ...
        print(
            f"    Response time: {statistics.mean(response_times):.2f}s ± {statistics.stdev(response_times) if len(response_times) > 1 else 0:.2f}s"
        )
        print(
            f"    Keywords matched: {statistics.mean(keyword_matches):.1f}/{results[0]['total_keywords']}"
        )
    ...
    # Save results
    output_file = Path(
        f"docs/benchmarks/sophia_quick_bench_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
