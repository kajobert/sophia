...
async def run_test(llm_tool: LLMTool, model_id: str, scenario: Dict, run_num: int) -> Dict:
    ...
    try:
        ...
        context.payload = {"prompt": scenario["prompt"], "model_config": {"model": model_id}}
    ...
    for category in set(s["category"] for s in TEST_SCENARIOS):
        ...
        if cat_results:
            category_scores[category] = (
                statistics.mean(r["avg_quality"] for r in cat_results) * 100
            )
...
async def main():
    ...
    # Print summary table
    print(
        f"{'Rank':<6} {'Model':<25} {'Size':<8} {'RAM':<10} {'Score':<8} {'Success':<9} {'Time':<8}"
    )
    print("-" * 80)
    ...
    for i, result in enumerate(all_results, 1):
        ...
        print(
            f"{rank_icon:<6} {result['model']:<25} {result['size']:<8} {result['ram']:<10} "
    ...
    for category in categories:
        ...
        cat_sorted = sorted(
            all_results, key=lambda x: x["category_scores"].get(category, 0), reverse=True
        )[:3]
