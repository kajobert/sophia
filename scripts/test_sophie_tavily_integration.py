...
async def test_sophie_tavily_integration():
    ...
    if tavily_plugin:
        print(f"✅ tool_tavily plugin loaded successfully")
    ...
    try:
        ...
        print(f"✅ Pydantic request model working:")
        print(f"   Type: {type(request).__name__}")
        print(f"   Query: {request.query}")
        print(f"   Validated: {request.search_depth}")
        
        # Verify response model structure
        print(f"✅ Pydantic response models defined:")
        print(f"   - TavilySearchResponse (query, results, answer, images)")
        print(f"   - TavilySearchResult (title, url, content, score)")
    ...
    if search_method:
        ...
        if "TavilySearchResponse" in str(return_annotation):
            print(f"   ✅ Returns Pydantic model (type-safe!)")
        else:
    ...
    if api_key:
        print(f"✅ TAVILY_API_KEY found in environment")
        print(f"   Key prefix: {api_key[:15]}...")
        print(f"   Ready for live API calls")
    else:
        print(f"⚠️  TAVILY_API_KEY not set in .env")
        print(f"   Plugin will work but API calls will fail")
        print(f"   Get your key at: https://tavily.com")
