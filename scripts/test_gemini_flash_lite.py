...
async def test_model(model_id: str, model_name: str, cost_per_1m: float, 
                     evaluator: ModelEvaluatorTool, context: SharedContext):
    """Test a single model."""
    print(f"\n{'='*80}")
    ...
    try:
        ...
        if score >= 8:
            print(f"   ✅ PASSED - Suitable for production!")
            if cost_per_1m < 0.14:
