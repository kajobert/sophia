...
def test_pydantic_models():
    ...
    # Test LLMCall
    call = LLMCall(
        timestamp=datetime.now(), model="gpt-4", input_tokens=100, output_tokens=50, cost=0.003
    )
    print(
        f"✅ LLMCall: {call.model}, {call.input_tokens + call.output_tokens} tokens, ${call.cost}"
    )

    # Test ToolUsage
    usage = ToolUsage(
        timestamp=datetime.now(), tool_name="tool_tavily", method_name="search", success=True
    )
    ...
    print(
        f"✅ Metrics: {metrics.total_llm_calls} calls, ${metrics.total_cost}, {metrics.success_rate*100}% success"
    )
...
def test_logging():
    ...
    result = plugin.log_tool_usage(
        context=context, tool_name="tool_tavily", method_name="search", success=True
    )
    print(f"✅ {result}")

    result = plugin.log_tool_usage(
        context=context, tool_name="tool_jules", method_name="create_session", success=True
    )
    print(f"✅ {result}")
