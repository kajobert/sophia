...
def load_benchmark_results() -> List[Dict[str, Any]]:
    ...
    for filename in os.listdir(BENCHMARK_DIR):
        if filename.endswith(".json") and "error" not in filename:
...
def calculate_price_performance(
    results: List[Dict[str, Any]], pricing_df: pd.DataFrame
) -> pd.DataFrame:
    ...
    merged_df["Cost (USD)"] = (
        (merged_df["input_tokens"] / 1_000_000) * merged_df["cost_prompt_usd_per_1m"].astype(float)
    ) + (
        (merged_df["output_tokens"] / 1_000_000)
        * merged_df["cost_completion_usd_per_1m"].astype(float)
    )

    # Calculate Price/Performance score
    # We add a small epsilon to the cost to avoid division by zero for free models.
    merged_df["Price/Performance"] = (merged_df["Overall Score"] * 1000) / (
        merged_df["Cost (USD)"] + 1e-9
    )
...
def generate_leaderboard(
    df: pd.DataFrame, title: str, cost_limit_lower: float, cost_limit_upper: float
) -> str:
    """Generates a Markdown leaderboard for a specific cost category."""
    df_filtered = df[
        (df["cost_prompt_usd_per_1m"] > cost_limit_lower)
        & (df["cost_prompt_usd_per_1m"] <= cost_limit_upper)
    ]
    ...
    # fmt: off
    report_df = report_df.rename(columns={"cost_prompt_usd_per_1m": "Cost (USD/1M Tokens)"})  # noqa: E501
    # fmt: on
