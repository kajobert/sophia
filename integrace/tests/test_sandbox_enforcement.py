...
def test_sandbox_blocks_network_requests():
    ...
    with pytest.raises(
        RuntimeError, match="Tests must not create network sockets in test mode!"
    ):
...
def test_sandbox_blocks_forbidden_file_writes():
    ...
    with pytest.raises(
        RuntimeError, match="Test tried to write outside of allowed directories"
    ):
...
def test_sandbox_blocks_subprocess():
    ...
    with pytest.raises(
        RuntimeError, match="Tests must not call 'subprocess.Popen' in test mode!"
    ):
        subprocess.Popen(["ls"])
...
def test_sandbox_blocks_os_system():
    ...
    with pytest.raises(
        RuntimeError, match="Tests must not call 'os.system' in test mode!"
    ):
...
def test_sandbox_blocks_time_sleep():
    ...
    with pytest.raises(
        RuntimeError, match="Tests must not call 'time.sleep' in test mode!"
    ):
        time.sleep(1)
...
def test_sandbox_blocks_env_var_changes():
    ...
    with pytest.raises(
        RuntimeError, match="Test must not change un-whitelisted env var: FORBIDDEN_VAR"
    ):
