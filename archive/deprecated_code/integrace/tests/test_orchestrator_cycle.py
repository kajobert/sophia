...
async def test_successful_plan_execution(orchestrator, mock_llm):
    ...
    orchestrator.tools["ListDirectoryTool"].execute.return_value = [
        "file1.txt",
        "file2.txt",
    ]
    orchestrator.tools[
        "WriteFileTool"
    ].execute.return_value = "File 'test.txt' has been written successfully."
    ...
    orchestrator.tools["WriteFileTool"].execute.assert_called_once_with(
...
async def test_plan_execution_with_failure_and_repair(orchestrator, mock_llm):
    ...
    failing_plan = [
        {
            ...
            "parameters": {"file_path": "non_existent_file.txt"},
        }
    ]
    ...
    corrected_plan = [
        {
            "step_id": 1,
            "description": "Write a file first",
            "tool_name": "WriteFileTool",
            "parameters": {"file_path": "new_file.txt", "content": "I exist now"},
        },
        {
            "step_id": 2,
            "description": "Now read the file",
            "tool_name": "ReadFileTool",
            "parameters": {"file_path": "new_file.txt"},
        },
    ]
    ...
    orchestrator.tools[
        "WriteFileTool"
    ].execute.return_value = "File 'new_file.txt' has been written successfully."
    ...
    orchestrator.tools["WriteFileTool"].execute.assert_called_once_with(
        file_path="new_file.txt", content="I exist now"
    )
