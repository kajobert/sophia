...
class StartMonitoringRequest(BaseModel):
    ...
    max_duration: int = Field(
        default=3600, ge=60, le=86400, description="Maximum monitoring duration in seconds"
    )
...
class CognitiveJulesMonitor(BasePlugin):
    ...
    def check_session_status(self, context: SharedContext, session_id: str) -> JulesSessionStatus:
        """
    ...
    def monitor_until_completion(
        ...
        try:
            request = MonitorUntilCompletionRequest(
                session_id=session_id, check_interval=check_interval, timeout=timeout
            )
        except Exception as e:
        ...
        while datetime.now() < max_time:
            ...
            if status.is_completed:
                ...
                return {"status": status, "results_applied": False, "mode": "api-only"}
            ...
            context.logger.info(
                f"Session {request.session_id} state: {status.state} " f"(elapsed: {elapsed:.0f}s)"
            )
        ...
        return {"status": status, "results_applied": False, "timeout": True, "mode": "api-only"}
    ...
    def get_monitoring_summary(self, context: SharedContext) -> Dict[str, Any]:
        ...
        completed_count = len(
            [t for t in self.active_monitors.values() if t.status == "completed"]
        )
