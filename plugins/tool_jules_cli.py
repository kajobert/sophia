...
class JulesCLIPlugin(BasePlugin):
    ...
    async def create_session(
        ...
        if not session_ids:
            ...
            context.logger.info(
                f"✅ Created {len(session_ids)} Jules session(s): {session_ids}"
            )
    ...
    async def pull_results(
        ...
        if request.apply:
            context.logger.info(f"✅ Jules results applied to local repository")
            return {
                "success": True,
                "applied": True,
                "output": output,
            ...
            context.logger.info(f"✅ Jules results retrieved (not applied)")
            return {
                "success": True,
                "applied": False,
                "diff": output,
