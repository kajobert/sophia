...
class CoreProcessManager(BasePlugin):
    ...
    def setup(self, config: dict) -> None:
        ...
        self.logger.info("Core Process Manager initialized", extra={"plugin_name": self.name})
            ...
            },
            {
                ...
                "function": {
                    ...
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "process_id": {"type": "string", "description": "Process ID to query"}
                        },
                        "required": ["process_id"],
                    },
                },
    ...
    async def start_background_process(
        ...
        try:
            ...
            monitor_task = asyncio.create_task(self._monitor_process(context, process, timeout))
            ...
            return {"success": False, "error": str(e), "message": f"Failed to start process: {e}"}

    async def _monitor_process(
        self, context: SharedContext, process: BackgroundProcess, timeout: int
    ):
        """
            ...
            if timeout > 0:
                try:
                    stdout, stderr = await asyncio.wait_for(
                        process.subprocess.communicate(), timeout=timeout
                    )
    ...
    async def get_process_status(self, context: SharedContext, process_id: str) -> Dict[str, Any]:
        """
    ...
    async def stop_background_process(
        self, context: SharedContext, process_id: str, force: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.info(f"Stopped process: {process.name}", extra={"plugin_name": self.name})
    ...
    async def list_background_processes(
        self, context: SharedContext, filter_state: str = "all"
    ) -> Dict[str, Any]:
        """
