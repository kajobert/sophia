...
Implements complete self-improvement loop: hypothesis → sandbox test → benchmark → deploy.
...
Inspired by evolutionary algorithms: generate → test → select → deploy.
...
class CognitiveSelfTuning(BasePlugin):
    ...
    def setup(self, config: Dict[str, Any]) -> None:
        ...
        self.logger.info("🧪 Cognitive Self-Tuning Plugin initializing...")
        ...
        if not self.db:
            self.logger.warning("⚠️  memory_sqlite plugin not found - hypothesis storage unavailable")
        
        # Load configuration from autonomy.yaml → self_improvement.self_tuning
        ...
        self.logger.info(f"📁 Sandbox directory: {self.sandbox_path}")
        
        # Log configuration
        self.logger.info(f"⚙️  Improvement threshold: {self.improvement_threshold*100}%")
        self.logger.info(f"🚀 Auto-deploy: {self.auto_deploy}")
        self.logger.info(f"🔄 Max concurrent tests: {self.max_concurrent_tests}")
        
        self.logger.info("✅ Cognitive Self-Tuning Plugin ready!")
    ...
    async def _on_hypothesis_created(self, event: Event):
        ...
        try:
            ...
            if not hypothesis_id:
                self.logger.error("❌ HYPOTHESIS_CREATED event missing hypothesis_id")
                return
            
            self.logger.info(f"🔔 New hypothesis received: ID={hypothesis_id}")
            
            # Check concurrent test limit
            if len(self.currently_testing) >= self.max_concurrent_tests:
                self.logger.warning(f"⚠️  Max concurrent tests ({self.max_concurrent_tests}) reached, queuing hypothesis {hypothesis_id}")
                return
    ...
    async def _process_hypothesis(self, hypothesis_id: int):
        ...
        try:
            self.logger.info(f"🧪 Testing hypothesis {hypothesis_id}...")
            ...
            self.logger.info(f"📦 Creating sandbox: {sandbox_dir}")
            ...
            if not success:
                self.logger.error(f"❌ Failed to apply fix in sandbox")
                self.db.update_hypothesis_status(
                    hypothesis_id=hypothesis_id,
                    status="rejected",
            ...
            if improvement >= self.improvement_threshold:
                self.logger.info(f"✅ Hypothesis APPROVED (improvement: {improvement*100:.2f}% >= {self.improvement_threshold*100}%)")
                ...
                if self.auto_deploy:
                    ...
                    if deploy_success:
                        ...
                        self.logger.info(f"🚀 Hypothesis {hypothesis_id} DEPLOYED!")
                    ...
                    self.logger.info(f"⏸️  Auto-deploy disabled, hypothesis awaiting manual deployment")
                    
            else:
                self.logger.info(f"❌ Hypothesis REJECTED (improvement: {improvement*100:.2f}% < {self.improvement_threshold*100}%)")
                self.db.update_hypothesis_status(
                    hypothesis_id=hypothesis_id,
                    status="rejected",
                    test_results={
            ...
            self.logger.error(f"❌ Error processing hypothesis {hypothesis_id}: {e}", exc_info=True)
            self.db.update_hypothesis_status(
                hypothesis_id=hypothesis_id,
                status="rejected",
        ...
        try:
            ...
            if fix_type == "code":
                ...
                if not source_file.exists():
                    self.logger.error(f"❌ Target file not found: {source_file}")
                    return False
                
                # Copy original file to sandbox
                shutil.copy2(source_file, sandbox_file)
                self.logger.info(f"📄 Copied {target_file} to sandbox")
                
                # Apply fix (replace entire file content)
                with open(sandbox_file, 'w', encoding='utf-8') as f:
                    f.write(proposed_fix)
                self.logger.info(f"✏️  Applied code fix to {sandbox_file}")
                ...
                try:
                    ...
                    self.logger.info(f"✏️  Applied config changes to {sandbox_file}")
                except yaml.YAMLError as e:
                    self.logger.error(f"❌ Invalid YAML in proposed fix: {e}")
                    return False
                ...
                try:
                    ...
                    self.logger.info(f"✏️  Applied model strategy changes")
                except yaml.YAMLError as e:
                    self.logger.error(f"❌ Invalid YAML in model fix: {e}")
                    return False
            else:
            ...
            self.logger.error(f"❌ Error applying fix in sandbox: {e}", exc_info=True)
            return False
    ...
    async def _run_benchmark(self, sandbox_dir: Path, fix_type: str, target_file: str) -> Dict[str, Any]:
        ...
        if fix_type == "code":
            ...
            self.logger.error(f"❌ Unknown fix type for benchmarking: {fix_type}")
    ...
    async def _benchmark_code(self, workspace_root: Path, sandbox_dir: Path, target_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # e.g., plugins/cognitive_reflection.py → test_reflection.py or test_phase_3_3_reflection.py
            test_files = []
            ...
            if not test_files:
                self.logger.warning(f"⚠️  No test files found for {target_file}, using conservative estimate")
            ...
            # Run baseline tests
            self.logger.info(f"🧪 Running baseline tests: {test_files}")
            ...
            self.logger.info(f"🧪 Running sandbox tests: {test_files}")
            ...
            self.logger.error(f"❌ Error in code benchmarking: {e}", exc_info=True)
    ...
    async def _run_pytest(self, test_files: List[Path], use_sandbox: bool, sandbox_dir: Optional[Path]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error("❌ Pytest timeout (120s)")
    ...
    async def _benchmark_prompt(self, workspace_root: Path, sandbox_dir: Path, target_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"🧪 Starting enterprise-grade prompt benchmarking...")
            self.logger.info(f"📏 Baseline: {len(baseline_prompt)} chars, New: {len(new_prompt)} chars")
            ...
            if real_world_result and real_world_result.get("success"):
                self.logger.info(f"✅ Real-world testing complete: {real_world_result['improvement_percentage']*100:.1f}% improvement")
                return real_world_result
            
            # Fallback to heuristic if real-world testing fails
            self.logger.warning("⚠️  Real-world testing failed, using heuristic fallback")
            ...
            self.logger.error(f"❌ Error in prompt benchmarking: {e}")
    ...
    async def _real_world_prompt_test(self, baseline_prompt: str, new_prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"📊 Loaded {len(failed_ops)} failed operations for testing")
            ...
            self.logger.info(f"🎯 Extracted {len(test_cases)} test cases")
            ...
            self.logger.info(f"📈 Baseline success: {baseline_success_rate*100:.1f}%, New: {new_success_rate*100:.1f}%")
            ...
            self.logger.error(f"❌ Real-world testing error: {e}", exc_info=True)
            ...
            for i, test_case in enumerate(test_cases):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        self.logger.debug(f"  Test {i+1}/{len(test_cases)}: {'✓' if success else '✗'}")
                    else:
    ...
    async def _benchmark_config(self, workspace_root: Path, sandbox_dir: Path, target_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"✅ Config YAML valid")
            ...
            self.logger.error(f"❌ Error in config benchmarking: {e}")
            return {"error": str(e), "improvement_percentage": 0.0}
    ...
    async def _deploy_fix(self, sandbox_dir: Path, target_file: str, hypothesis: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not sandbox_file.exists():
                self.logger.error(f"❌ Sandbox file not found: {sandbox_file}")
                return False
            ...
            if production_file.exists():
                ...
                self.logger.info(f"💾 Backed up original to {backup_file}")
            ...
            try:
                ...
                self.logger.info(f"✅ Git commit created")
                ...
                self.logger.warning(f"⚠️  Git commit failed (may not be in git repo): {e}")
            ...
            if "plugins/" in target_file or "core/" in target_file:
                self.logger.warning(f"⚠️  Critical file modified: {target_file}")
                self.logger.info(f"🔄 Triggering autonomous upgrade validation...")
            ...
            self.logger.error(f"❌ Error deploying fix: {e}", exc_info=True)
            return False
    ...
    async def _create_pull_request_for_deployment(self, hypothesis: Dict[str, Any], target_file: str):
        ...
        try:
            ...
            if not github_plugin:
                self.logger.warning("⚠️  GitHub plugin not available, skipping PR creation")
                return
            ...
            if not github_config.get("enabled", False):
                self.logger.info("ℹ️  GitHub integration disabled in config")
                return
            ...
            try:
                ...
                self.logger.warning("⚠️  Could not determine current branch")
                return
            
            # Don't create PR if already on target branch
            if current_branch == base_branch:
                self.logger.info(f"ℹ️  Already on {base_branch}, skipping PR")
                return
...
"""
            ...
            self.logger.info(f"📝 Creating PR: {current_branch} → {base_branch}")
            ...
            self.logger.info(f"✅ PR created: {pr_response.html_url}")
            self.logger.info(f"📋 PR #{pr_response.number}: {pr_response.title}")
            ...
            if self.db:
                try:
                    ...
                    self.logger.warning(f"⚠️  Could not update hypothesis with PR info: {e}")
    ...
    async def _trigger_autonomous_upgrade_validation(
        ...
        try:
            ...
            self.logger.info(f"💾 Upgrade state saved to {upgrade_state_file}")
            self.logger.info(f"🔄 Requesting graceful restart for upgrade validation...")
            ...
            self.logger.info(f"✅ Restart requested, Guardian will handle graceful shutdown")
            ...
            self.logger.error(f"❌ Error triggering upgrade validation: {e}", exc_info=True)
    ...
    async def _validate_upgrade(self, upgrade_state: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.info(f"🧪 Validating upgrade for hypothesis {hypothesis_id}...")
            ...
            if "plugins/" in target_file:
                ...
                if not plugin_init_result:
                    self.logger.error(f"❌ Plugin initialization failed for {target_file}")
                    return False
            ...
            if not test_result:
                self.logger.error(f"❌ Validation tests failed for {target_file}")
                return False
            ...
            if not regression_check:
                self.logger.error(f"❌ Regression detected after upgrade")
                return False
            
            # All validations passed
            self.logger.info(f"✅ Upgrade validation PASSED")
            ...
            self.logger.error(f"❌ Upgrade validation error: {e}", exc_info=True)
            return False
    ...
    async def _rollback_deployment(self, upgrade_state: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.warning(f"⚠️  ROLLBACK INITIATED for hypothesis {hypothesis_id}")
            self.logger.info(f"📦 Restoring {backup_file} → {target_file}")
            ...
            self.logger.info(f"✅ Backup restored")
            ...
            try:
                ...
                self.logger.info(f"✅ Rollback commit created")
            except subprocess.CalledProcessError as e:
                self.logger.warning(f"⚠️  Rollback commit failed: {e}")
            ...
            self.logger.info(f"✅ Rollback complete, requesting restart with original code...")
    ...
    async def _check_plugin_initialization(self, target_file: str) -> bool:
        ...
        try:
            ...
            if plugin_name in all_plugins:
                ...
                self.logger.warning(f"⚠️  Plugin {plugin_name} not in all_plugins")
                return True  # Don't fail on this alone
                
        except Exception as e:
            self.logger.error(f"❌ Plugin check error: {e}")
            return False
    ...
    async def _run_validation_tests(self, target_file: str) -> bool:
        ...
        try:
            ...
            if "plugins/" in target_file:
                # plugins/cognitive_self_tuning.py → test_phase_3_4_self_tuning.py
                plugin_name = Path(target_file).stem
            ...
            if not test_file:
                self.logger.warning(f"⚠️  No test file found for {target_file}, skipping validation tests")
                return True  # Don't fail if no tests exist
            
            # Run tests using pytest
            self.logger.info(f"🧪 Running validation tests: {test_file}")
            ...
            if result.returncode == 0:
                self.logger.info(f"✅ Validation tests PASSED")
                return True
            else:
                self.logger.error(f"❌ Validation tests FAILED:\n{result.stdout}\n{result.stderr}")
                return False
                
        except subprocess.TimeoutExpired:
            self.logger.error(f"❌ Validation tests TIMEOUT")
            return False
        except Exception as e:
            self.logger.error(f"❌ Validation test error: {e}")
            return False
    ...
    async def _check_for_regressions(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"✅ No regressions detected (startup successful)")
            return True
            
        except Exception as e:
            self.logger.warning(f"⚠️  Regression check error: {e}")
    ...
    async def _cleanup_sandbox(self, sandbox_dir: Path):
        ...
        try:
            if sandbox_dir.exists():
                ...
                self.logger.info(f"🗑️  Cleaned up sandbox: {sandbox_dir}")
        except Exception as e:
            self.logger.warning(f"⚠️  Failed to cleanup sandbox {sandbox_dir}: {e}")
