...
class InterfaceTerminalStarTrek(BasePlugin):
    ...
    def display_message_stream(self, role: str, content_generator, **kwargs):
        ...
        with self.console.status(
            "[bold blue]◉[/] [yellow]COMPUTER:[/] [dim blue]processing...[/]", spinner="dots"
        ) as status:
            time.sleep(2)
    ...
    def get_user_input(self, prompt: str = "Enter command") -> str:
        ...
        try:
            user_input = Prompt.ask(
                f"[dim blue][SD {stardate}][/] [bold bright_yellow]CREW[/] [orange1]▌[/orange1]"
            )
            return user_input
        except:
...
async def demo():
    ...
    await interface.stream_text_async(
        "Engage! Set course for the future, maximum warp!", delay=0.02
    )
    ...
    interface.console.print(
        "[dim blue][SD 2025.1103.2046][/] [bold bright_yellow]CREW:[/] [bright_yellow]Computer, what is our current status?[/]"
    )
