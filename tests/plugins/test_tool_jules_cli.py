...
class TestJulesCLIPlugin:
    ...
    def plugin(self):
        ...
        # Mock bash tool with execute_command method (not execute)
        bash_tool = Mock()
        bash_tool.execute_command = AsyncMock(return_value="")

        # Setup plugin with mocked bash (dependency injection format)
        config = {
            "logger": logging.getLogger("test_jules_cli"),
    ...
    async def test_create_session_single(self, plugin, context):
        ...
        call_args = plugin.bash_tool.execute_command.call_args
    ...
    @pytest.mark.asyncio
    async def test_create_session_parallel(self, plugin, context):
        """Test creating parallel Jules sessions"""
        # Mock bash output with multiple session IDs (string format)
        plugin.bash_tool.execute_command.return_value = "Created sessions: 123456, 123457, 123458"

        # Create parallel sessions
        result = await plugin.create_session(
            context, repo="ShotyCZ/sophia", task="Complex refactoring task", parallel=3
        )
    ...
    @pytest.mark.asyncio
    async def test_create_session_validation_error(self, plugin, context):
        """Test validation error for invalid parallel value"""
        with pytest.raises(Exception):  # Pydantic validation error
            await plugin.create_session(
                context, repo="ShotyCZ/sophia", task="Test task", parallel=10  # Invalid: max is 5
            )

    @pytest.mark.asyncio
    async def test_create_session_bash_failure(self, plugin, context):
        """Test handling bash execution failure"""
        # Mock bash failure (execute_command raises exception)
        plugin.bash_tool.execute_command.side_effect = Exception("Authentication required")

        # Create session
        result = await plugin.create_session(context, repo="ShotyCZ/sophia", task="Test task")
    ...
    @pytest.mark.asyncio
    async def test_pull_results_view_only(self, plugin, context):
        """Test pulling results without applying"""
        # Mock bash output (diff) - execute_command returns string
        diff_output = """
...
+    return True
"""
        plugin.bash_tool.execute_command.return_value = diff_output

        # Pull results
        result = await plugin.pull_results(context, session_id="123456", apply=False)
    ...
    @pytest.mark.asyncio
    async def test_pull_results_with_apply(self, plugin, context):
        """Test pulling and applying results"""
        # Mock bash output (string format)
        plugin.bash_tool.execute_command.return_value = "Changes applied successfully"

        # Pull and apply results
        result = await plugin.pull_results(context, session_id="123456", apply=True)

        # Verify result
        assert result["success"] is True
        assert result["applied"] is True
        assert "output" in result
        assert "applied" in result["output"].lower() or "message" in result
    ...
    @pytest.mark.asyncio
    async def test_pull_results_session_id_cleanup(self, plugin, context):
        """Test that 'sessions/' prefix is removed from session ID"""
        plugin.bash_tool.execute_command.return_value = "diff output"

        # Pull with full session ID format
        result = await plugin.pull_results(context, session_id="sessions/123456", apply=False)
    ...
    @pytest.mark.asyncio
    async def test_list_sessions(self, plugin, context):
...
"""
        plugin.bash_tool.execute_command.return_value = list_output

        # List sessions
        result = await plugin.list_sessions(context)
    ...
    @pytest.mark.asyncio
    async def test_list_sessions_empty(self, plugin, context):
        """Test listing when no sessions exist"""
        plugin.bash_tool.execute_command.return_value = "No sessions found"
    ...
    def test_get_tool_definitions(self, plugin):
        ...
        # Check specific tools exist (no prefix - plugin returns simple names)
        tool_names = [t["function"]["name"] for t in definitions]
        assert "create_session" in tool_names
        assert "pull_results" in tool_names
