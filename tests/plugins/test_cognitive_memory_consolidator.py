...
class TestMemoryStorage:
    ...
    async def test_store_consolidated_memories_counts_by_type(self, consolidator):
        ...
        result = ConsolidationResult(
            insights=[
                ExtractedMemory(
                    memory_type=MemoryType.INSIGHT, summary="Insight 1", importance=3, keywords=[]
                ),
                ExtractedMemory(
                    memory_type=MemoryType.INSIGHT, summary="Insight 2", importance=4, keywords=[]
                ),
            ],
            patterns=[
                ExtractedMemory(
                    memory_type=MemoryType.PATTERN, summary="Pattern 1", importance=3, keywords=[]
                )
            ],
            facts=[
                ExtractedMemory(
                    memory_type=MemoryType.FACT, summary="Fact 1", importance=5, keywords=[]
                )
            ],
        )
...
class TestToolExecution:
    ...
    async def test_execute_tool_trigger_consolidation(self, consolidator, mock_context, event_bus):
        ...
        result = await consolidator.execute_tool(
            "trigger_memory_consolidation", {"force": False}, mock_context
        )
    ...
    async def test_execute_tool_get_status(self, consolidator, mock_context):
        ...
        result = await consolidator.execute_tool("get_consolidation_status", {}, mock_context)
    ...
    async def test_execute_tool_search_memories_not_implemented(self, consolidator, mock_context):
        ...
        result = await consolidator.execute_tool(
            "search_consolidated_memories", {"query": "test"}, mock_context
        )
    ...
    async def test_execute_tool_unknown_tool(self, consolidator, mock_context):
        ...
        result = await consolidator.execute_tool("unknown_tool", {}, mock_context)
