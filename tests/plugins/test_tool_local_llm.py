...
from plugins.tool_local_llm import LocalLLMTool, LocalModelConfig
    ...
    async def test_generate_ollama_success(self, local_llm):
        ...
        with patch.object(local_llm.client, "post", new=AsyncMock(return_value=mock_response)):
            result = await local_llm.generate(
                prompt="Test prompt", system_prompt="You are helpful", temperature=0.7
            )
...
class TestToolDefinitions:
    ...
    def test_execute_local_llm_tool_schema(self, local_llm):
        ...
        tools = local_llm.get_tool_definitions()
        exec_tool = next(t for t in tools if t["function"]["name"] == "execute_local_llm")
...
class TestToolExecution:
    ...
    async def test_execute_tool_execute_local_llm(self, local_llm, mock_context):
        ...
        with patch.object(local_llm, "generate", new=AsyncMock(return_value="Generated text")):
            result = await local_llm.execute_tool(
                "execute_local_llm", {"prompt": "Test prompt", "temperature": 0.5}, mock_context
            )
    ...
    async def test_execute_tool_execute_local_llm_error(self, local_llm, mock_context):
        ...
        with patch.object(
            local_llm, "generate", new=AsyncMock(side_effect=Exception("Connection failed"))
        ):
            result = await local_llm.execute_tool(
                "execute_local_llm", {"prompt": "Test"}, mock_context
            )
    ...
    async def test_execute_tool_check_status(self, local_llm, mock_context):
        ...
        with patch.object(
            local_llm, "check_availability", new=AsyncMock(return_value=True)
        ), patch.object(
            local_llm, "list_models", new=AsyncMock(return_value=["gemma2:2b", "llama3:8b"])
        ):

            result = await local_llm.execute_tool("check_local_llm_status", {}, mock_context)
    ...
    async def test_execute_tool_unknown_tool(self, local_llm, mock_context):
        ...
        result = await local_llm.execute_tool("unknown_tool", {}, mock_context)
