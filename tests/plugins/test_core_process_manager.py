...
def context(event_bus):
    ...
    ctx = SharedContext(session_id="test-session", current_state="TESTING", logger=Mock())
...
def test_background_process_creation():
    ...
    process = BackgroundProcess(
        process_type=ProcessType.TEST_SUITE, name="Test Process", command="pytest tests/"
    )
...
async def test_stop_background_process(process_manager, context):
    ...
    result = await process_manager.start_background_process(
        context=context, process_type="custom", name="Long Running", command="sleep 30", timeout=0
    )
    ...
    stop_result = await process_manager.stop_background_process(
        context=context, process_id=process_id, force=False
    )
...
async def test_list_background_processes(process_manager, context):
    ...
    await process_manager.start_background_process(context, "build", "Build 1", "echo 'build1'", 5)
    await process_manager.start_background_process(context, "custom", "Custom 1", "sleep 5", 0)
...
async def test_concurrent_processes(process_manager, context):
    ...
    completed = [
        p for p in process_manager.processes.values() if p.state == ProcessState.COMPLETED
    ]
