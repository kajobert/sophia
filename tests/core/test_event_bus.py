...
async def test_priority_ordering():
    ...
    bus.publish(Event(event_type=EventType.CUSTOM, priority=EventPriority.LOW, source="test"))
    bus.publish(Event(event_type=EventType.CUSTOM, priority=EventPriority.CRITICAL, source="test"))
    bus.publish(Event(event_type=EventType.CUSTOM, priority=EventPriority.NORMAL, source="test"))
...
async def test_async_handler():
    ...
    bus.publish(Event(event_type=EventType.TASK_CREATED, source="test", data={"value": "done"}))
...
async def test_sync_handler():
    ...
    bus.publish(Event(event_type=EventType.TASK_STARTED, source="test", data={"value": "started"}))
...
async def test_event_history():
    ...
    for i in range(10):
        bus.publish(Event(event_type=EventType.CUSTOM, source=f"test{i}", data={"index": i}))
...
async def test_event_immutability():
    ...
    event = Event(event_type=EventType.USER_INPUT, source="test", data={"key": "value"})
...
async def test_concurrent_publish():
    ...
    tasks = []
    for i in range(100):
        event = Event(event_type=EventType.CUSTOM, source=f"test{i}", data={"index": i})
        bus.publish(event)
