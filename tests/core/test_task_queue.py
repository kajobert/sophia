...
async def test_task_priority_ordering(task_queue):
    ...
    normal_task = Task(name="normal", function=record_task, args=("normal",), priority=TaskPriority.NORMAL)
    high_task = Task(name="high", function=record_task, args=("high",), priority=TaskPriority.HIGH)
    critical_task = Task(name="critical", function=record_task, args=("critical",), priority=TaskPriority.CRITICAL)
