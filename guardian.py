...
def load_config():
    ...
    try:
        ...
        log_message(f"CHYBA: Chyba při parsování konfiguračního souboru '{CONFIG_FILE}': {e}")
        return {}
...
def main():
    ...
    log_message(f"Nastavené prahové hodnoty: CPU > {cpu_threshold}%, RAM > {ram_threshold}%")
    ...
    while True:
        ...
        while process.poll() is None:
            ...
            if cpu_usage > cpu_threshold or ram_usage > ram_threshold:
                ...
                log_message(f"VAROVÁNÍ: Překročení prahové hodnoty ({threshold_breach_counter}/3). CPU: {cpu_usage}%, RAM: {ram_usage}%")
                if threshold_breach_counter >= 3:
                    log_message(f"KRITICKÉ: Prahová hodnota překročena 3x po sobě. Provádím měkký restart.")
